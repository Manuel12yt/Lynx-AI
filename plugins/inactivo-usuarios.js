let handler = async (m, { conn, text, args, groupMetadata }) => {
    ...
    for (let i = 0; i < member.length; i++) {
        ...
        if (!args[0]) {
        return conn.reply(m.chat, `❄️ Utilice el comando con opciones:\n1. \`.inactivos list\` para etiquetar los miembros off\n2. \`.inactivos kick\` para expulsar a miembros off`, m,rcanal)
    }

    if (args[0] === 'list') {
        if (total === 0) return conn.reply(m.chat, `❄️ *No hay bandos en este grupo.*`, m,rcanal)
    ...
    if (args[0] === 'kick') {
        if (total === 0) return conn.reply(m.chat, `❄️ *No hay bandos para patear en este grupo.*`, m,rcanal)
        ...
        return conn.reply(m.chat, `❄️ Eliminado con éxito *${total}* miembros inactivos del grupo.`, m,rcanal)
    }

    return conn.reply(m.chat, `❄️ Opción no válida. Utilice \`list\` para ver miembros inactivos o \`kick\` para eliminarlos.`, m,rcanal)
}
